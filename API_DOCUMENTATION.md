...
All four workflow writes (including POST/PUT aliases) optionally accept JSON `expectedVersion`, a non-negative safe integer copied from the report's `__v` when it was viewed, or `0` when a legacy report has no `__v`. This is not the content `version`. A mismatch returns `409 REPORT_CONFLICT`; null, strings, fractions, negative numbers, and unsafe integers return `400 INVALID_REPORT_VERSION`. Authorization, state, and existing payload validation can reject a request first. Denied writes leave the report, history, audit, and notifications unchanged. For example, approval can send `{ "expectedVersion": 2 }`. Omission remains backward-compatible, including body-less submission/approval, but does not protect stale browser views. Clients must retain the viewed token and require explicit reconsideration after a conflict, not fetch a fresh token and silently retry.

The Report Generator browser screen sends the viewed `__v` as `expectedVersion` for submission, approval, and rejection, and adopts the returned counter after success. Missing legacy counters use zero; malformed counters block actions. A conflict clears the rejection draft and blocks decisions until an explicit reload succeeds; failed reloads remain blocked and writes are never automatically retried. Report switching, generation, and duplicate decisions are blocked during a workflow write. There is no content-edit action on this screen. The API precondition remains optional for other callers.

Delete races, direct database assignment changes that bypass versioning, immutable release artifacts, complete historical approval metadata, source snapshots, and draft export markings remain separate requirements. Audit and notification writes are not transactional with the report save. Regression tests cover version preconditions on every write alias, legacy zero tokens, revisions, revoked assignments, sequential repeated decisions, and deterministic concurrent edit/submit/approve/reject races using isolated MongoDB and real JWT authentication. Browser regressions cover token propagation, conflict reload and failure, invalid counters, duplicate writes, and desktop/mobile layout. Existing persistence and browser CI discovery includes these tests; the full export workflow remains unverified.
